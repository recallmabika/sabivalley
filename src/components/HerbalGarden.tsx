...
import { Leaf, Sparkles, ShieldCheck, Flower2 } from "lucide-react";
    ...
    <section id="garden" className="py-24 bg-[#FDFCFA]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          >
            <span className="inline-flex items-center rounded-full bg-[#1B3B2B]/10 px-3 py-1 text-sm font-semibold text-[#1B3B2B] ring-1 ring-inset ring-[#1B3B2B]/20 mb-4">
              Botanical Heritage
            </span>
              ...
              {highlights.map((item, index) => {
                ...
                return (
                  <div key={index} className="p-5 rounded-xl bg-[#F4EFEA] border border-[#1B3B2B]/5 flex flex-col justify-start">
                    <div className="w-10 h-10 rounded-lg bg-[#1B3B2B] text-[#FDFCFA] flex items-center justify-center mb-4 shrink-0 shadow-sm">
                      <IconComponent className="w-5 h-5 text-[#C27D38]" />
                    </div>
